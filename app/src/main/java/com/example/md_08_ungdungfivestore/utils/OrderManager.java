...
public class OrderManager {
    ...
    public void createOnlineOrder(OrderRequest request, OrderCallback callback) {
        orderService.createOnlineOrder(request).enqueue(new Callback<OrderResponse>() {
            @Override
            public void onResponse(Call<OrderResponse> call, Response<OrderResponse> response) {
                Log.d(TAG, "Response Code for createOnlineOrder: " + response.code());
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (response.code() == 401) {
                        ...
                        String errorBody = response.errorBody() != null ? response.errorBody().toString() : "Lỗi không xác định";
                        errorMsg = "Lỗi tạo đơn hàng VNPAY: Mã lỗi " + response.code() + ". Chi tiết: " + errorBody;
                    }
                    Log.e(TAG, errorMsg);
                    callback.onError(errorMsg);
                }
            ...
            @Override
            public void onFailure(Call<OrderResponse> call, Throwable t) {
                Log.e(TAG, "createOnlineOrder failure: " + t.getMessage(), t);
                callback.onError("Lỗi mạng hoặc kết nối (VNPAY): " + t.getMessage());
            }
        });
    }
}